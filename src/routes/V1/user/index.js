...
const USER_ROUTE = [
  ...
  {
    method: 'GET',
    path: '/user/v1/list-saved-wallet',
    options: {
      handler: async (request, reply) => {
        try {
          request.query.language = request.headers['accept-language'] || LANGUAGE.EN;
          let dataToSend = await UserControllers.listSavedWallet(request.auth.credentials.data, request.query);
          return sendSuccess(
            STATUS_MSG.SUCCESS.DEFAULT,
            dataToSend,
            request.headers['accept-language'] || LANGUAGE.EN,
  ...
  {
    method: 'GET',
    path: '/user/v1/list-referred-users',
    options: {
      handler: async (request, reply) => {
        try {
          request.query.language = request.headers['accept-language'] || LANGUAGE.EN;
          let dataToSend = await UserControllers.listReferredUsers(request.auth.credentials.data, request.query);
          return sendSuccess(
            STATUS_MSG.SUCCESS.DEFAULT,
            dataToSend,
            request.headers['accept-language'] || LANGUAGE.EN,
